  ...
  useEffect(() => {
    ...
    for (let i = 0; i <= n; i++) {
      ...
      for (let j = 0; j <= i; j++) {
        ...
        const upCount = i - j;
        const downCount = j;
        let yOffset = nodeRadius + 15;
        
        if (i === 0) {
          ctx.fillText('S₀', x, y + yOffset);
        } else if (upCount === 0) {
          ctx.fillText(`S₀·d^${downCount}`, x, y + yOffset);
        } else if (downCount === 0) {
          ctx.fillText(`S₀·u^${upCount}`, x, y + yOffset);
        } else {
          ctx.fillText(`S₀·u^${upCount}·d^${downCount}`, x, y + yOffset);
        }
        ...
        if (i === n) {
          ...
          // 초기 노드: 최종 결과
          ctx.fillText(`최종 ${optionLabel} = ${optionValue.toFixed(2)}`, x, y + yOffset);
        } else {
          // 중간 노드: 할인된 기댓값
          ...
          ctx.fillText(`=${discount.toFixed(4)}·[${p.toFixed(3)}·${upValue.toFixed(2)}+${(1-p).toFixed(3)}·${downValue.toFixed(2)}]`, x, y + yOffset);
          yOffset += 12;
          ...
          if (isEarlyExercise) {
            ...
            ctx.fillText(`행사=${exerciseValue.toFixed(2)} > 보유`, x, y + yOffset);
          } else {
